...
class MultiMAE(nn.Module):
    ...
    def forward(self, 
        ...
        if 'rgb' in x:
            ...
            B, C, H, W = x['ired'].shape
        elif 'sired' in x:
            B, C, H, W = x['sired'].shape
        elif 'ebands' in x:
            B, C, H, W = x['ebands'].shape
        elif 'semseg' in x:
            B, H, W = x['semseg'].shape
            H *= self.input_adapters['semseg'].stride_level
            W *= self.input_adapters['semseg'].stride_level
        elif 'semsegdw' in x:
            B, H, W = x['semsegdw'].shape
            H *= self.input_adapters['semsegdw'].stride_level
            W *= self.input_adapters['semsegdw'].stride_level
        elif 'input_ids' in x:
            inputs_ids = x['input_ids']
        elif 'attention_mask' in x:
...
class MultiViT(MultiMAE):
    ...
    def process_input(self, x):
        ...
        if 'rgb' in x:
            ...
            B, _, H, W = x['ired'].shape
        elif 'sired' in x:
            B, _, H, W = x['sired'].shape
        elif 'ebands' in x:
            B, _, H, W = x['ebands'].shape
        elif 'semseg' in x:
            B, H, W = x['semseg'].shape
            H *= self.input_adapters['semseg'].stride_level
            W *= self.input_adapters['semseg'].stride_level
